...
docker-compose exec -T clickhouse clickhouse-client \
  ...
  pv -l -s $DATA_SIZE | \
  docker-compose exec -T kafkacat kafkacat -X "security.protocol=sasl_ssl" -X "sasl.mechanism=PLAIN" -X "sasl.username=client" -X "sasl.password=client-secret" -X "sasl.ca.location=secrets/"  -b broker:9092 -t dummytopic -P -l
