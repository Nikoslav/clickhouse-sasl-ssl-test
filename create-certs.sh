...
# Generate CA key
openssl req -new -x509 -keyout /tmp/snakeoil-ca-1.key -out /tmp/snakeoil-ca-1.crt -days 365 -subj '/CN=clickhouse/OU=Test/O=Clickhouse/L=Location/ST=State/C=CH' -passin pass:confluent -passout pass:confluent

for i in broker clickhouse zookeeper
do
    echo "------------------------------- $i -------------------------------\n"
    ...
    keytool -genkey -noprompt \
                 -alias $i \
                 -dname "CN=$i,OU=Test,O=Clickhouse,L=Location,S=State,C=CH" \
